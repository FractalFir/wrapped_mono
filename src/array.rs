    ...
    [(); DIMENSIONS as usize]: Copy,
{
    ...
    pub unsafe fn from_ptr(ptr: *mut MonoArray) -> Option<Self> {
        ...
        #[cfg(not(feature = "unsafe_arrays"))]
        {
            ...
            assert!(
                rank == DIMENSIONS,
                "Array dimension mismatch got:{}, expected:{}",
                rank,
                DIMENSIONS
            );
    ...
    pub fn new(domain: &Domain, size: &[usize; DIMENSIONS as usize]) -> Self {
        ...
        let arr = unsafe {
            Self::from_ptr(crate::binds::mono_array_new_full(
                domain.get_ptr(),
                class.get_ptr(),
                size as *const [usize] as *mut usize,
                null_mut(),
            ))
        }
...
impl<T: InteropSend + InteropRecive + InteropClass, const DIMENSIONS: u32> InteropRecive
    for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
...
impl<T: InteropSend + InteropRecive + InteropClass, const DIMENSIONS: u32> InteropSend
    for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
...
impl<T: InteropSend + InteropRecive + InteropClass, const DIMENSIONS: u32> InteropClass
    for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
...
impl<T: InteropSend + InteropRecive + InteropClass, const DIMENSIONS: u32>
    crate::object::ObjectTrait for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
    fn hash(&self) -> i32 {
...
impl<T: InteropSend + InteropRecive + InteropClass, const DIMENSIONS: u32> Clone
    for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
...
impl<const DIMENSIONS: u32, T: InteropSend + InteropRecive + InteropClass, O: ObjectTrait>
    PartialEq<O> for Array<DIMENSIONS, T>
where
    [(); DIMENSIONS as usize]: Copy,
{
