use crate::Assembly;
use crate::metadata::{TypeDefinition,Method};
...
impl BindingGenerator{
    pub fn create<P:AsRef<Path>>(path:P)->Result<Self,BindgenError>{
        ...
        let mut target = std::fs::OpenOptions::new().write(true).read(true).create(true).open(path)?;
        write!(target,"// C# bindings generated by wrapped_mono 0.3.2\n#![allow(dead_code)]\n");
        Ok(Self{assemblies:Vec::new(),target,namespaces_out:HashMap::new()})
    }
    ...
    pub fn generate(mut self)->Result<(),BindgenError>{
        for assembly_index in 0..self.assemblies.len(){
            ...
            self.generate_types(tdt,&asm_name)?;
        }
    ...
    pub fn generate_types(&mut self,tdt:TypeDefinitionTable,asm_name:&str)->Result<(),BindgenError>{
        for td in tdt.defs(){
            let name = td.name();
            if name.contains('<'){
                continue;
