    ...
    pub fn wrapped(inner: &Self) -> Self {
        ...
        let res = unsafe {
            Self::from_ptr(crate::binds::mono_get_exception_runtime_wrapped(
                inner.get_ptr() as *mut MonoObject,
            ))
        }
    ...
    pub unsafe fn from_ptr(exc_ptr: *mut MonoException) -> Option<Self> {
        ...
        #[cfg(feature = "referneced_objects")]
        {
            if exc_ptr.is_null() {
                ...
                Some(Self {
                    handle: GCHandle::create_default(exc_ptr as *mut _),
                })
            }
        }
    }
...
impl core::fmt::Debug for Exception {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        let mstr = self
            ...
            .expect("Got null intead of a string while tring to convert an exception to a string!")
            .to_string();
